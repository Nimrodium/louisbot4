...
from cmd_parser import CommandResult, parse
from config import Config
from database import DB, BatchCache, Server
...
def can_delete_lock(lock: str) -> bool:
    with open(lock) as f:
        ...
        if raw.isdigit():
            ...
            raise Exception(
                f"could not read database lock file: {lock} contained a non-integer PID"
            )
        if os.getpid() == pid:
            ...
            print(
                f"lock file {lock} found but the owner process has since died, safe to proceed"
            )
            return True
        else:
            print(
                f"lock file {lock} exists and is not stale, process {pid} is still running!\nCannot run two instances of louisbot on the same database!!"
            )
            return False
...
class Scraper(commands.Bot):
    def __init__(self, cfg: Config):
        ...
        self.bridges = Bridges(cfg.bridges)
            ...
            for channel_id, unix_stamp in channels.items():
                ...
                if isinstance(channel, Messageable):
                    ...
                    messages = channel.history(
                        limit=None, after=datetime.datetime.fromtimestamp(unix_stamp)
                    )
                    i = 0
                    print()
                    async for message in messages:
                        print(
                            f"\033[F \33[2K\rprocessing message: {i}, channel: {channel.name}"
                        )
                        stdout.flush()
                        i += 1
                        user = message.author
                        # print(f"{i}: {user.name}: {message.content}")
                        db_server.update_user_msg_count(
                            user.id, user.name, message.created_at, 1
                        )  # bit stupid i have to increment by one, could fix later by collecting all then storing, eh its python aint gonna be efficent
                        for reaction in message.reactions:
                            if isinstance(reaction.emoji, str):
                                emoji_name = reaction.emoji
                            else:
                                emoji_name = reaction.emoji.name
                            db_server.update_user_emoji_count(
                                user.id,
                                user.name,
    ...
    async def on_message(self, message: Message):
        print(
            f"MESSAGE: {message.author.name}: {message.content}"
        ) if message.author.id != (
            self.user.id if self.user is not None else None
        ) else None
        if message.guild and not message.author.bot:
            self.batch.log_pointer(
                message.guild.id, message.channel.id, message.created_at.timestamp()
            )
            self.batch.fill += 1
        ...
        if message.content.startswith(self.cfg.prefix):
            ...
            if all(
                map(
                    lambda c: True if c == self.cfg.prefix else False,
                    # removes all whitespaces
                    [
                        c
                        for c in msg_no_prefix
                        if c
                        not in [
                            " ",
                            "\t",
                ...
                mapping = map(
                    lambda c: True if c == self.cfg.prefix else False, no_whitespace
                )
                print(
                    f'{msg_no_prefix} contained only prefix characters\n\tno whitespace "{no_whitespace}"\n\tmapping: "{list(mapping)}"\n'
                )
                return
            # await self.process_batch()
            if message.guild is None:
                await send_internal_error(
                    f"command with content {message.content} by {message.author.name} had no associated guild"
                )
                return
            server_name: str | None = self.cfg.get_server_alias(message.guild.id)
            if server_name is None:
                await send_internal_error(
                    f'`cfg.get_server_alias("{message.guild.id}")` returned `None`, the server is not tracked. perhaps you forgot to add it to the config?'
                )
            ...
            if server is None:
                await send_internal_error(
                    f'`self.open_server("{server_name}")` returned `None`'
                )
                print(self.servers.keys())
                return
            response = parse(
                message.content.removeprefix(self.cfg.prefix),
                message.author.id,
                server,
                server_name,
                cfg,
            ...
            if not response.do_not_send:
                ...
                print(
                    f"sending message {msg} from {response} {f'containing image {response.img}' if response.img else ''}"
                )
