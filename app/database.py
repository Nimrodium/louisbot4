...
class Day:
    ...
    def to_dict(self) -> dict:
        return {
            self.MSG_HOURS: self.msg_hours,
            self.EMOJI_HOURS: self.emoji_hours,
            self.DATE: self.date,
        }
    ...
    def from_dict(cls, d: dict) -> "Day":
        try:
            return cls(
                date=d[cls.DATE],
                msg_hours=d[cls.MSG_HOURS],
                emoji_hours=d[cls.EMOJI_HOURS],
            )
...
def build_file_path(server_name: str, year: int) -> str:
    return os.path.join(
        server_name, build_file_name(os.path.basename(server_name), year)
    )
...
class Server:
    ...
    def update_user_emoji_count(
        self, id: int, name: str, hr: int, emoji: str, new_emojis_count: int
    ):
