        ...
        if os.path.exists(CONFIG):
            with open(CONFIG, "r") as f:
                ...
                try:
                    ...
                    self.dump_interval_minutes: int = int(
                        cfg[self.KEY_DUMP_INTERVAL_MINUTES]
                    )
                    self.tracked_servers: dict[str, str] = cfg[self.KEY_TRACKED_SERVERS]
                    # print(self.server_names)
                    self.bridges: list[Bridge] = Bridge.get_bridges(cfg[self.BRIDGES])
                except KeyError as e:
                    print(f"missing key `{e}`")
                    exit()
                self.time_stamp_file = os.path.join(
                    self.database_directory, "timestamp"
                )
...
def interactive_setup():
    ...
    cfg[Config.KEY_DUMP_INTERVAL_MINUTES] = checked_int_input(
        f"{Config.KEY_DUMP_INTERVAL_MINUTES}"
    )
