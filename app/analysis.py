...
class ColorConfig:
    def __init__(self, cfg: Config):
        ...
        if os.path.exists(self.file_path):
            self.file = json.load(open(self.file_path, "r"))
        else:
            self.file = {}
    ...
    def get_color_list(self, users: list[db.User]):
        return [
            self.file[str(m.id)]
            if str(m.id) in self.file
            else f"#{hex(random.randint(0x333333, 0xDDDDDD)).removeprefix('0x')}"
            for m in users
        ]
...
class Analyzer:
    ...
    # in epoch days
    def collect_data_from_x_to_y(
        self, server: str, start: int, end: int
    ) -> list[db.User]:
        ...
        match cmd[0]:
            case "past" | "last" | "p" | "l":
                match cmd[2]:
                    case "day" | "days" | "d":
                        if cmd[1].isdigit():
                            start = db.datetime_to_epoch_day(
                                datetime.datetime.now()
                                - datetime.timedelta(days=int(cmd[1]))
                            )
            ...
            case "from" | "f":
                ...
                start = db.datetime_to_epoch_day(
                    parser.parse(" ".join(start_human_readable_list))
                )
                end = db.datetime_to_epoch_day(
                    parser.parse(" ".join(cmd[len(start_human_readable_list) + 2 :]))
                )
                return (start, end)
            ...
            case "pie" | "p":
                match cmd[2]:
                    case "msgs" | "m":
                        ...
                        msg, attachment = self.generate_message_pie_chart(
                            server, start, end
                        )
                        await message.channel.send(msg, file=nextcord.File(attachment))
                    case "emoji" | "e":
                        start, end = self.get_start_end(cmd[4:])
                        msg, attachment = self.generate_emoji_pie_chart(
                            server, cmd[3], start, end
                        )
                        await message.channel.send(msg, file=nextcord.File(attachment))
            case "line" | "l":
                print("line")
                ...
                match cmd[4]:
                    ...
                    case "all" | "a":
                        ...
                        match cmd[2]:
                            case "messages" | "msgs" | "m":
                                msg, attachment = self.generate_line_message_chart(
                                    server, users, start, end, granularity
                                )
                                await message.channel.send(
                                    msg, file=nextcord.File(attachment)
                                )
                            # case "emojis"|"emoji"|"e":
                    ...
                    case "users" | "user" | "u":
                        ...
                        for word in cmd[5:]:
                            ...
                            if len(word) > 32:
                                await message.channel.send(
                                    f"{word} is an invalid username (too long {len(word)}>32)"
                                )
                                return
                            if word == ";":
                                break
                                ...
                                for c in word:
                                    if (
                                        c not in string.ascii_lowercase
                                        and c not in string.digits
                                        and c not in ["_", "."]
                                    ):
                                        await message.channel.send(
                                            f"{word} is an invalid username (invalid character {c})"
                                        )
                                        return
                        ...
                        match cmd[2].split(":")[0]:
                            ...
                            case "emojis" | "emoji" | "e":
                                emoji = cmd[2].split(":")[1]
                                msg, attachment = self.generate_line_emoji_chart(
                                    server, users, start, end, granularity, emoji
                                )
                                await message.channel.send(
                                    msg, file=nextcord.File(attachment)
                                )
    ...
    def generate_line_message_chart(
        self, server: str, users: list[db.User], start: int, end: int, granularity: str
    ) -> tuple[str, str | None]:
        ...
        match granularity:
            case "hours" | "hr":
                ...
                return self.generate_line_chart(
                    f"Messages over Hours from {db.epoch_to_unix(start).strftime('%m/%d/%Y')} to {db.epoch_to_unix(end).strftime('%m/%d/%Y')}",
                    "",
                    "Hours",
                    "Messages",
                    names,
            ...
            case "days" | "d":
                ...
                users_x: list[int] = list(range(len(users_y[0])))
                return self.generate_line_chart(
                    f"Messages over Days from {db.epoch_to_unix(start).strftime('%m/%d/%Y')} to {db.epoch_to_unix(end).strftime('%m/%d/%Y')}",
                    "",
                    "Days",
                    "Messages",
                    names,
    ...
    def generate_line_emoji_chart(
        ...
        #         users.append(user)
        users = sorted(
            self.collect_data_from_x_to_y(server, start, end),
            key=lambda u: u.sum(),
            reverse=True,
        )
    ...
    def generate_line_chart(
        ...
        fig, ax = plt.subplots()
        print(
            f"generating line chart {title} x: {xlabel} y:{ylabel} lbls:{line_labels}"
        )
        print(f"x: {x} \n\nys: {y}")
    ...
    def get_users_list(
        self, all_users: list[db.User], included_users: list[db.User]
    ) -> list[db.User]:
        return [u for u in all_users if u.name in map(lambda u: u.name, included_users)]

    def generate_message_pie_chart(
        self, server: str, users: list[db.User], start: int, end: int
    ) -> tuple[str, str | None]:
        all_users = sorted(
            self.collect_data_from_x_to_y(server, start, end),
            key=lambda u: u.sum(),
    ...
    def generate_emoji_pie_chart(
        self, server: str, emoji: str, users: list[db.User], start: int, end: int
    ) -> tuple[str, str | None]:
        all_users = sorted(
            self.collect_data_from_x_to_y(server, start, end),
            key=lambda u: u.sum_emoji(emoji),
            reverse=True,
        )
        users = self.get_users_list(all_users, users)
        labels = [u.name for u in users]
    ...
    def generic_pie_chart(
        ...
        for n in range(
            users_to_display
        ):  # so no more redundant calls for total messages which could get expensive on albeit extremely large datasets
            readout += f"\t{labels[n]}: {values[n]}{suffix}\n"
        readout += (
            "use `!setcolor #<hexcolor>` to set a custom color for your pie slice"
        )
...
def get_fig_path() -> str:
    return os.path.join(
        os.environ["TEMP"] if os.name == "nt" else "/tmp", ".louisbot_plot.png"
    )
